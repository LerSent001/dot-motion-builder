...
export function ColorOpacityInput({
  ...
  return (
    <InputGroup className="w-[68px] flex-none rounded-l-none [&:not(:focus-within):hover]:!border-[color:color-mix(in_oklab,var(--border)_20%,transparent)] [&:not(:focus-within):hover]:text-[color:var(--foreground)]">
      <InputGroupInput
        aria-label={`${label}不透明度`}
        autoComplete="off"
        className="pl-1.5 pr-0.5 text-right font-mono text-[10px]"
        inputMode="numeric"
        name={name}
      ...
      <InputGroupAddon align="inline-end" className="pr-1.5 pl-0">
        <InputGroupText className="text-[10px] group-hover/input-group:text-[color:var(--foreground)]">
          %
        </InputGroupText>
      </InputGroupAddon>
    </InputGroup>
